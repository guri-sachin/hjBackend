...
const BlogUpload = () => {
  ...
  const froalaConfig = {
    imageUploadURL: 'http://localhost:3001/upload_image', // Use your actual API URL
    imageUploadParams: {
      id: 'my_editor'
    },
    toolbarButtons: [
  ...
  const handleSubmit = async () => {
    try {
      const response = await axios.post('http://localhost:3001/api/save_blog', { content: froalaContent });
